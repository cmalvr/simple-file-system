...
int main( ) {
  ...
  for (i = 0; i < 2; i++) {
    ...
    for (j = 0; j < filesize[i]; j += chunksize) {
      ...
      if (readsize != chunksize) {
        fprintf(stderr, "ERROR: Requested %d bytes, read %d, from file: %d\n", chunksize, readsize, i);
        readsize = chunksize;
      }
  ...
  for (i = 0; i < nopen; i++) {
    ...
    if (fds[i] >= 0) {
      ...
      if (readsize != strlen(test_str)) {
        fprintf(stderr, "ERROR: Read wrong number of bytes: Should be:%lu, but we got:%d\n",strlen(test_str),readsize);
        error_count++;
      }
   ...
   */
    ...
    if (fds[i] >= 0) {
      readsize = sfs_fread(fds[i], fixedbuf, sizeof(fixedbuf));
      if (readsize < strlen(test_str)) {
        fprintf(stderr, "ERROR: Read wrong number of bytes: %d\n", readsize);
        error_count++;
      }
      ...
      for (j = 0; j < strlen(test_str);j++) {
        if (test_str[j] != fixedbuf[j]) {
          fprintf(stderr, "ERROR: Wrong byte in %s at position %d (%d,%d) at file %d\n", names[i], j, fixedbuf[j],test_str[j],i);
          error_count++;
          break;
        }
      }
